...
% 04191050
% BBC Model dengan data input simulasi PV
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% Perhitungan tegangan output berdasarkan dari PV yang mengacu terhadap data iradiasi dan temperatur yang berubah
% clear all;close all;clc
% Input data primer excel dan sekunder excel ke dalam PV model lalu simpan dalam bentuk variabel VoutPV dan IoutPV di workspace kemudian masukkan kedalam Model BBC
...
Ppv = 50; % Daya PV
Vin = VoutPV; % Tegangan PV (V) >> Vin = VoutPV(1,1);
Iin = IoutPV; % Arus PV (A)
...
Vin = VoutPV(n); % Tegangan PV (V) 
Iin = IoutPV(n); % Arus PV (A)
Pin = Vin*Iin; % Daya keluaran PV (W)
Vout = 12; % Tegangan baterai (V)
Iout = 2; % Arus baterai (A)
Pout = Vout*Iout; % Daya baterai (W)
Fs = 20000; % Frekuensi switching (Hz)
dVout = Vout*0.5; % Ripple tegangan output(V)
Ts = 1/Fs; % Periode switching (s)
